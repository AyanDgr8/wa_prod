        ...
        const [result] = await connection.query(
            `UPDATE register 
             SET password = ?, 
                 ...
                 company_gst = ?
             WHERE email = ?`,
            [hashedPassword, card_select, card_detail, address, company_name, company_gst, email]
        );

        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Registration not found. Please complete the payment process first.' });
        }
